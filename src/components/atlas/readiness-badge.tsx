 ...
 *
 * Backend readiness values remain stable for compatibility, while the
 * inventor-facing labels explain what the state actually means.
 */
...
const stateLabels: Record<ReadinessState, string> = {
  ...
  "Getting There": "Evidence building",
  "Ready to Move Forward": "Ready for next stage",
};
...
export function ReadinessBadge({ state, className }: ReadinessBadgeProps) {
  return (
    <span
      ...
      title={`Readiness: ${stateLabels[state]}`}
    >
      {stateLabels[state]}
    </span>
  );
