...
function downloadMarkdown(title: string, version: number, content: string) {
  ...
  const anchor = document.createElement("a"); anchor.href = url; anchor.download = `${safeName}-v${version}.md`; anchor.click(); URL.revokeObjectURL(url);
}
...
export default function InventionWorkPage() {
  ...
  const packageExport: AtlasPackageExport = {
    ...
    deliverables: latestDeliverables.map((deliverable) => ({
      ...
      confidence: deliverable.confidence, searchDate: deliverable.searchDate, staleReason: deliverable.staleReason, artifactMaturity: deliverable.artifactMaturity,
      assumptions: deliverable.assumptions, limitations: deliverable.limitations, missingInformation: deliverable.missingInformation ?? [],
      sources: deliverable.sourceIds.flatMap((sourceId) => { const source = sourcesById.get(String(sourceId)); return source ? [{ id: String(source._id), title: source.title, locator: source.locator, reliability: source.reliability }] : []; }),
      reviews: library.reviews.filter((review) => review.deliverableId === deliverable._id).map((review) => ({ specialty: review.specialty, status: review.status, scope: review.scope, reviewerName: review.reviewerName, reviewerReference: review.reviewerReference, notes: review.notes })),
    })),
  };
  const packageSafetyError = validatePackageExportSafety(packageExport.deliverables, packageExport.qualityPassed, packageExport.qualityBlockers);
