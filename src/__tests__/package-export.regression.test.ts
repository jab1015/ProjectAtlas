...
const representativePackage: AtlasPackageExport = {
  ...
  qualityBlockers: [],
  deliverables: [{
    kind: "invention_brief", title: "Invention Brief", version: 2,
    trustState: "ready_for_authorized_use", trustLabel: "Ready for authorized use",
    content: "A countertop rack supports produce above a sink while rinsing.\n\nThis is representative pilot content.",
    sourceCoverage: 0.75, confidence: 0.82, searchDate: 1_725_000_000_000, artifactMaturity: "engineering_reviewed",
    assumptions: ["Target sinks use common residential dimensions."], limitations: ["No production release is implied."], missingInformation: [],
    sources: [{ id: "source-1", title: "Representative source", locator: "https://example.com/source", reliability: "authoritative_secondary" }],
    reviews: [{ specialty: "engineering", status: "accepted", scope: "Representative professional review." }],
  }],
};

function installDownloadCapture() {
  const blobs: Blob[] = []; const filenames: string[] = []; const originalDocument = globalThis.document;
  const originalCreateObjectURL = URL.createObjectURL; const originalRevokeObjectURL = URL.revokeObjectURL;
  URL.createObjectURL = vi.fn((blob: Blob | MediaSource) => { if (blob instanceof Blob) blobs.push(blob); return "blob:inventsmith-test"; }); URL.revokeObjectURL = vi.fn();
  Object.defineProperty(globalThis, "document", { configurable: true, value: { createElement: () => ({ href: "", set download(value: string) { filenames.push(value); }, click: vi.fn() }) } });
  return { blobs, filenames, restore() { URL.createObjectURL = originalCreateObjectURL; URL.revokeObjectURL = originalRevokeObjectURL; if (originalDocument === undefined) delete (globalThis as { document?: Document }).document; else Object.defineProperty(globalThis, "document", { configurable: true, value: originalDocument }); } };
}
afterEach(() => { vi.restoreAllMocks(); });

describe("InventSmith package export", () => {
  it("creates a filesystem-safe descriptive filename", () => { expect(buildAtlasPackageFilename("Sink Rack: Version 2!", "pdf")).toBe("sink-rack-version-2-inventsmith-feasibility-package.pdf"); });
  ...
  it("blocks package export when automated package quality has not passed", () => {
    expect(validatePackageExportSafety(representativePackage.deliverables, false, ["Required artifact missing"])).toMatch(/quality checks have not passed/i);
  });
  it("blocks stale or not-explicitly-authorized deliverables from package export", () => {
  ...
  it("generates a structurally valid PDF only for an authorized package", async () => {
    const capture = installDownloadCapture(); try { await exportAtlasPackagePdf(representativePackage); expect(capture.blobs).toHaveLength(1); expect(capture.filenames[0]).toBe("adjustable-produce-rinsing-rack-inventsmith-feasibility-package.pdf"); const bytes = new Uint8Array(await capture.blobs[0].arrayBuffer()); expect(bytes.length).toBeGreaterThan(1_000); expect(new TextDecoder().decode(bytes.slice(0, 8))).toMatch(/^%PDF-/); } finally { capture.restore(); }
  }, 15_000);
});
