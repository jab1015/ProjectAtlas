...
const cadSpecSchema = {
  ...
  properties: {
    ...
    parts: { type: "array", minItems: 1, maxItems: 24, items: { type: "object", additionalProperties: false, properties: {
      id: { type: "string" }, name: { type: "string" }, material: { type: "string" }, primitiveType: { type: "string", enum: ["box", "cylinder", "tube"] },
      sizeX: { type: "number" }, sizeY: { type: "number" }, sizeZ: { type: "number" }, radius: { type: "number" }, outerRadius: { type: "number" }, innerRadius: { type: "number" }, height: { type: "number" }, segments: { type: "integer" }, position: vecSchema, rotationDeg: vecSchema,
...
interface ModelPart { id: string; name: string; material: string; primitiveType: "box" | "cylinder" | "tube"; sizeX: number; sizeY: number; sizeZ: number; radius: number; outerRadius: number; innerRadius: number; height: number; segments: number; position: [number, number, number]; rotationDeg: [number, number, number]; }
...
function toPart(part: ModelPart): CadPartSpec {
  const base = { id: part.id.trim().slice(0, 80), name: part.name.trim().slice(0, 160), material: part.material.trim().slice(0, 160), position: part.position.map((value, index) => bounded(value, -5000, 5000, `part position ${index + 1}`)) as [number, number, number], rotationDeg: part.rotationDeg.map((value) => Math.max(-360, Math.min(360, value))) as [number, number, number] };
  ...
  return { ...base, primitive: { type: "tube", outerRadius: bounded(part.outerRadius, 0.1, 2500, `${part.name} outerRadius`), innerRadius: bounded(part.innerRadius, 0.05, 2499, `${part.name} innerRadius`), height: bounded(part.height, 0.1, 5000, `${part.name} height`), segments: Math.max(12, Math.min(96, Math.round(part.segments || 48))) } };
}
