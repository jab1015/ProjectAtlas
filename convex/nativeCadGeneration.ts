...
const cadSpecSchema = {
  ...
  properties: {
    ...
    parts: { type: "array", minItems: 1, maxItems: 24, items: { type: "object", additionalProperties: false, properties: {
      id: { type: "string" }, name: { type: "string" }, material: { type: "string" }, finish: { type: "string" }, manufacturingProcess: { type: "string" }, interfaceNotes: { type: "array", items: { type: "string" } }, primitiveType: { type: "string", enum: ["box", "cylinder", "tube", "frustum", "threadedCylinder", "threadedTube", "revolvedProfile", "extrudedConvexPolygon"] },
...
interface ModelPart {
  ...
  primitiveType: "box" | "cylinder" | "tube" | "frustum" | "threadedCylinder" | "threadedTube" | "revolvedProfile" | "extrudedConvexPolygon";
  sizeX: number;
  sizeY: number;
  sizeZ: number;
  radius: number;
...
function toPart(part: ModelPart): CadPartSpec {
  ...
  if (part.primitiveType === "threadedTube") return { ...base, primitive: { type: "threadedTube", outerRadius: bounded(part.outerRadius, 0.5, 2500, `${part.name} outer radius`), innerRadius: bounded(part.innerRadius, 0.25, 2499, `${part.name} minimum inner radius`), height: bounded(part.height, 1, 5000, `${part.name} height`), pitch: bounded(part.pitch, 0.25, 250, `${part.name} internal thread pitch`), threadDepth: bounded(part.threadDepth, 0.05, 100, `${part.name} internal thread depth`), segments: Math.max(16, Math.min(96, Math.round(part.segments || 48))) } };
  if (!Array.isArray(part.polygonPoints) || part.polygonPoints.length < 3 || part.polygonPoints.length > 32) throw new Error(`${part.name} requires 3–32 profile points`);
  if (part.primitiveType === "revolvedProfile") {
    const points = part.polygonPoints.map(([radius, z], index) => [bounded(radius, 0.05, 2500, `${part.name} profile point ${index + 1} radius`), bounded(z, -2500, 2500, `${part.name} profile point ${index + 1} z`)] as Vec2);
    return { ...base, primitive: { type: "revolvedProfile", points, segments: Math.max(16, Math.min(96, Math.round(part.segments || 48))) } };
  }
  ...
  handler: async (ctx, args) => {
    try {
      ...
      const response = await client.responses.create({
        ...
        input: [
          { role: "system", content: "You are the InventSmith CAD planning engine. Convert the evidence-backed selected product design into a conservative preliminary parametric assembly using the supported deterministic geometry: box, cylinder, hollow tube, tapered circular frustum, helical externally threaded cylinder, helical internally threaded tube/sleeve, closed annular revolved radius/Z profile, and extrusion of a CONVEX 2D polygon. Use millimeters. Prefer geometry that preserves the selected mechanism, patent-design differentiation constraints, interfaces, assembly intent, sealing intent, and manufacturability rather than reducing every product to generic boxes and cylinders. For every part, preserve the best-supported material, finish intent, target manufacturing process, and concise interface/mating notes from the selected design and engineering/manufacturing specifications. If any of those fields are unknown, say TBD or provisional rather than inventing certainty. Use threadedCylinder for screw-driven shafts, threaded plungers, twist-lift actuation, or other external helical interfaces. Use threadedTube for a mating internally threaded collar, sleeve, cap, nut-like component, or other internal helical interface. Mating external/internal thread pairs should use compatible nominal pitch while explicitly recording clearance, thread form, backlash, wear, and fit as unresolved unless engineering evidence supports them. Use revolvedProfile for axisymmetric annular components or interface features such as sealing lips, gasket/O-ring gland forms, annular grooves, stepped collars, shoulders, retaining features, and rotationally symmetric custom cross-sections. For revolvedProfile, polygonPoints are ordered [radius,z] points forming one closed, non-self-intersecting annular cross-section and every radius must remain greater than zero. A concave profile is allowed when needed for a groove or lip. Seal groove depth, elastomer compression/squeeze, gland fill, contact pressure, running clearance, fit and tolerance values remain provisional unless supported by engineering evidence and must be recorded in assumptions/interfaceNotes/unresolvedEngineering. Thread pitch and depth remain provisional unless supported by engineering evidence. Use frustums for simple tapered rotational forms and extrudedConvexPolygon for non-circular prismatic custom outlines. For geometry fields not used by a selected primitive, return harmless numeric defaults and an empty polygonPoints array unless the primitive uses polygonPoints. Never invent a critical dimension as if verified. When evidence does not support a dimension, pitch, clearance, seal interface, finish requirement, manufacturing process, or tolerance, choose a clearly provisional value where geometry requires one and record the uncertainty in assumptions, interfaceNotes, and unresolvedEngineering. Avoid impossible overlaps when practical. This output is deterministically converted into real STEP/STL/DXF artifacts and engineering views and remains Preliminary CAD requiring engineering/prototype review before manufacturing release." },
          { role: "user", content: JSON.stringify({ invention: { title: context.invention.title, problemStatement: context.invention.problemStatement, targetAudience: context.invention.targetAudience, solutionDescription: context.invention.solutionDescription }, structuredRecord: context.structuredBrief, currentDesignArtifacts: context.deliverables, currentEvidenceFindings: context.findings, supportedGeometry: { units: "mm", primitives: { box: "sizeX/sizeY/sizeZ", cylinder: "radius/height", tube: "outerRadius/innerRadius/height", frustum: "bottomRadius/topRadius/height", threadedCylinder: "radius/height/pitch/threadDepth; external helical thread", threadedTube: "outerRadius/innerRadius/height/pitch/threadDepth; internal helical bore/sleeve thread", revolvedProfile: "polygonPoints[[radius,z],...]; closed simple annular cross-section for grooves/lips/shoulders/collars", extrudedConvexPolygon: "polygonPoints[[x,y],...]/height; convex profiles only" }, partMetadata: { material: "best-supported material or provisional/TBD", finish: "finish intent or provisional/TBD", manufacturingProcess: "target process or provisional/TBD", interfaceNotes: "mating, sealing, assembly, clearance, service, or unresolved-interface notes" }, maxParts: 24 } }) },
        ],
