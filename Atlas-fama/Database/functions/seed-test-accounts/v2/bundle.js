...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Verify mode: attempt an actual password login for each account
    if (body.action === 'verify') {
      const anonKey = (Deno.env.get('SUPABASE_ANON_KEY') || '').trim() || svcKey;
      ...
      for (const acct of ACCOUNTS) {
        ...
        checks.push({
          email: acct.email,
          loginOk: res.ok && !!data.access_token,
          status: res.status,
          error: res.ok ? undefined : JSON.stringify(data).slice(0, 200)
        });
      }
      return new Response(JSON.stringify({ version: VERSION, mode: 'verify', checks }), {
        headers: { 'Content-Type': 'application/json', ...corsHeaders }
      });
    }
